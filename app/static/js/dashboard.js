(function () {
    ...
    async function handleRemoveScoobyClick() {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                setAddScoobyStatus(msg || 'Failed to remove Scooby from meeting.', 'error');
                return;
            }
            ...
            if (eventSource) {
                try {
                    ...
                    console.warn('Error closing EventSource on removal', e);
                }
                eventSource = null;
            }

            const nextState = Object.assign({}, state);
            delete nextState.botId;
            saveState(nextState);
        } catch (e) {
            console.error('Error calling /remove_scooby', e);
            setAddScoobyStatus('Error removing Scooby from meeting.', 'error');
        } finally {
    ...
    (function initFromState() {
        ...
        if (state.botId) {
            setAddScoobyStatus('Restored active Scooby. Bot ID: ' + state.botId, 'ok');
            startSummaryStream();
        }
    })();
