...
async def analyze_screen_from_slack(request: Request):
    ...
    async def process_and_send_analysis():
        """Background task to fetch latest screen analysis from pipeline and send to Slack"""
        try:
            ...
            # Fetch the latest screen analysis generated by the summarization pipeline
            analysis_text = await get_latest_screen_analysis_text()

            if not analysis_text:
                analysis_text = (
                    "No screenshare analysis is available yet. "
                    "Please make sure the meeting has progressed far enough for "
                    "screenshare-related segments to be processed."
                )
