...
class ScreenshareStorage:
    ...
    def _sanitize(self, text: Optional[str]) -> str:
        if not text:
            return "unknown"
        # Simple, filesystem-safe sanitization
    ...
    def save_png_frame(
        ...
        *,
        ...
        """Save a single PNG frame to disk and return the file path."""
        meeting_dir = self.ensure_meeting_dir(org_name, bot_id)

        # Build timestamp string for filename
        if timestamp_absolute:
            # Absolute is usually ISO string
            ts = timestamp_absolute.replace(":", "").replace("-", "").replace(".", "_").replace("Z", "z")
        else:
