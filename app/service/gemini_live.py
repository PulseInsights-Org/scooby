...
class GeminiLive():
    ...
    async def query_gemini_text(self, text: str) -> str:
        """Process text queries using WebSocket to Gemini Live API and return complete response."""
        async with self.client.aio.live.connect(
            model=self.model,
            config=self.config,
        ) as connection:
            ...
            async for n, response in self._async_enumerate(turn):
                if response.server_content:
                    ...
                    if transcription:
                        transcribed_text = getattr(transcription, "text", None)
                        if transcribed_text:
                            full_response += transcribed_text
                
                elif response.tool_call:
                    # Handle tool calls (retrieval tools)
                    try:
                        function_responses = []
                        for fc in response.tool_call.function_calls:
                            function_name = fc.name
                            function_args = fc.args
                            print(f"Function called by Gemini: {function_name}")
                            
                            try:
                                if function_name == "pc_retrieval_tool":
                                    query = function_args.get("query")
                                    data = self.tool_executor.pc_retrieval_tool(query)
                                elif function_name == "connections_retrieval_tool":
                                    event = function_args.get("event_names")
                                    data = self.tool_executor.get_event_connections(event)
                                else:
                                    data = {"error": f"Unknown function: {function_name}"}
            ...
            return full_response.strip() if full_response.strip() else "No response generated"
